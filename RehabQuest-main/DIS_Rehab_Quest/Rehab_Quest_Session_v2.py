"""
╔══════════════════════════════════════════════════════════════════╗
║        REHAB QUEST — Full Session Orchestrator  v2.0            ║
║  3 Sets × 3 Exercises × 120 s each                              ║
...
VIDEO_FILES = {
    'hand_raise':   os.path.join(BASE_DIR, 'Hand_Raise.mp4'),
    'wrist_circle': os.path.join(BASE_DIR, 'Wrist.mp4'),
    'leg_raise':    os.path.join(BASE_DIR, 'Leg_Raise.mp4'),
}
QUEUE_COLORS = {
...
def _load_apple_sprite(radius: int):
    ...
    raw = cv2.imread(os.path.join(BASE_DIR, 'Apple.png'), cv2.IMREAD_UNCHANGED)
    if raw is None:
...
while cap.isOpened():
    ...
    if phase == Phase.HAND_SELECT:
        ...
        if phase==Phase.EXERCISE and now>=phase_end: advance_phase()
